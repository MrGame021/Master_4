...
if(count($erros)>0){
    ...
    // ---------- FOTO: ----------
    $nome_ft = 'usuarios/default.jpg'; // valor padrão
    if(isset($_FILES['foto']) && $_FILES['foto']['erro'] === UPLOAD_ERR_OK){
    ...
    // ---------------------------
    $sql = "INSERT INTO usuarios (NOME, SENHA, DATA_NASC, EMAIL, GENERO, FOTO) VALUES ('$nome', '$senhaCrip', '$date', '$email', '$gen', '$nome_foto')";
    $conect->query($sql);
